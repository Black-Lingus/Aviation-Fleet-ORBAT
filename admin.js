...
function createIntelNodeElement(id, heading, content, width, height) {
  ...
  if (width !== undefined && width !== null) card.style.width = typeof width === 'string' ? width : `${width}px`;
  if (height !== undefined && height !== null) card.style.height = typeof height === 'string' ? height : `${height}px`;
  ...
  card.innerHTML = `
    ...
    <div class="node-body">
      ...
      <div class="node-live-preview">${parseMarkdownStyles(content)}</div>
    </div>
  `;
    ...
    try {
      ...
      if (!src.startsWith('data:')) {
        try {
          ...
          try { await loadImage(src); } catch (err2) { return resolve(src); }
        }
      } else {
        await loadImage(src);
      }
...
function createIntelTitleElement(id, heading, width, height) {
  ...
  const cssWidth = width.toString().includes('%') || width.toString().includes('px') ? width : `${width}px`;
  const cssHeight = height.toString().includes('%') || height.toString().includes('px') ? height : `${height}px`;
  if (width !== undefined && width !== null) bar.style.width = cssWidth;
  if (height !== undefined && height !== null) bar.style.height = cssHeight;
  bar.innerHTML = `
    <div class="intel-node-header">
...
async function saveIntelDataState() {
  const grid = document.getElementById('intel-board-grid');
  if (!grid) return;
